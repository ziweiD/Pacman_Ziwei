...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # computes the most likely position of each ghost that
        ...
        minDis = sys.maxsize
        cloestGhostPosition = None
        for p in mostLikelyPositions:
            dis = self.distancer.getDistance(p, pacmanPosition)
            if dis < minDis:
                minDis = dis
                cloestGhostPosition = p

        # find an action
        action = None
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            dis = self.distancer.getDistance(successorPosition, cloestGhostPosition)
            if dis < minDis:
                minDis = dis
                action = a
